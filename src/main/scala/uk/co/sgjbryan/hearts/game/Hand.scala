...
object Hand {
  ...
  final case class TrickComplete(winner: Player, cards: List[Card])
      extends Message
...
class Hand private (
  ...
  def calculatePoints(cardsTaken: Map[Player, List[Card]]): Map[Player, Int] = {
    val pointsTaken = cardsTaken.view
      .mapValues(cards => (cards map settings.scoreValue).sum)
      .toMap
    if (
      pointsTaken exists { case (_, score) => score == settings.shootValue }
    ) {
      pointsTaken.view
        .mapValues(score =>
          if (score == settings.shootValue) 0 else settings.shootValue
        )
        .toMap
    } else {
      pointsTaken
  ...
  def points(cards: List[Card]): Map[Card, Int] =
    cards.map(c => (c, settings.scoreValue(c))).toMap

  def passing(passes: List[(Player, List[Card])] = List()): Behavior[Message] =
    Behaviors.receivePartial { case (context, Pass(cards, player)) =>
      val newPasses = passes :+ (player, cards)
      if (newPasses.size == players.size) {
        newPasses foreach { case (passTo, pass) =>
          passTo.actor ! Seat.ReceivePass(pass)
        }
  ...
  def playing(
      order: List[Player],
      tricksRemaining: Int,
      cardsTaken: Map[Player, List[Card]] = Map()
  ): Behavior[Message] = Behaviors.receivePartial {
    case (context, TrickComplete(winner, cards)) =>
      val newCardsTaken = cardsTaken.updatedWith(winner)(cardsWon =>
        Some(cardsWon.getOrElse(List()) ++ cards)
      )
      val trickPoints = points(cards)
      val scoringCardsInTrick = (trickPoints filterNot { _._2 == 0 }).keySet
      val pointsInTrick = trickPoints.values.sum
      players map { _.actor } foreach {
        _ ! Seat.TrickEnded(winner, scoringCardsInTrick, pointsInTrick)
      }
